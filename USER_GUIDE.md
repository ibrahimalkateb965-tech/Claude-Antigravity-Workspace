...
2. [التخصيص والتصدير الذكي لطاقم المشروع (الخطاف 3)](#2-التخصيص-والتصدير-الذكي-لطاقم-المشروع-الخطاف-3)
3. [استخدام الخطاف رقم 20 لاستيراد الموارد البرمجية](#3-استيراد-الموارد-البرمجية-عالمياً-الخطاف-20)
4. [بوابات الحراسة الثلاث (The Three Guard Gates)](#4-بوابات-الحراسة-الثلاث)
5. [بروتوكول "الخطوة صفر" وإدارة الذاكرة](#5-بروتوكول-الخطوة-صفر-وإدارة-الذاكرة)
6. [المسار المتقدم لإنشاء المهارات والوكلاء الجدد](#6-المسار-المتقدم-لإنشاء-المهارات-والوكلاء-الجدد)
7. [المزامنة الآلية والتحديثات السحابية (الخطاف 21)](#7-المزامنة-الآلية-والتحديثات-السحابية-الخطاف-21)

---

## 1. نظام الخطافات التلقائية

الخطافات (Hooks) هي نظام التشغيل التلقائي للمنظومة (21 خطافاً تخصصياً). بدلاً من كتابة برومبتات طويلة ومعقدة في كل مرة، يكفي كتابة **الكلمة المحفزة (Trigger)** في بداية رسالتك ليقوم المحرك بالتعرف على نيتك وتفعيل الطاقم المناسب تلقائياً.
...
#### 🎯 بناء سياق المشروع وتخصيص بيئة الوكلاء
* **ما تكتبه:** `"سياق المشروع"` ثم `"اعتمد سياق المشروع وتخصيص وكلاء المشروع"`
* **ما يحدث خلف الكواليس:** 
  1. يقوم `prompt-engineer` ببناء ومطابقة `PROJECT_CONTEXT.md`.
  2. يُفعّل الخطاف رقم 3 مستدعياً `project_agent_tailor.py` لتحليل التقنيات وتصدير حزم الوكلاء المطلوبة فقط وحذف الفائض (Pruning).
...
* **ما يحدث خلف الكواليس:**
  ...
  2. يُفعّل "محامي الشيطان" لتدقيق الخطة لـ 7 مرات متتالية وتأمين قسم الآثار الجانبية `[UNINTENDED SIDE-EFFECTS]`.
...
## 2. التخصيص والتصدير الذكي لطاقم المشروع (الخطاف 3)

عند بدء العمل على مشروع مستقل جديد (مثل تطبيق أندرويد، أو مشروع بايثون، أو نظام ويب):

* **ما تكتبه:**
  ```text
  تخصيص وكلاء المشروع : صدّر بيئة المشروع ونظف المهارات الزائدة
  ```
* **ما تفعله المنظومة تلقائياً:**
  1. يحلل محرك `project_agent_tailor.py` ملف `PROJECT_CONTEXT.md` وملفات المشروع الفعلية.
  2. يحدد حزم الوكلاء المطلوبة ويصدرها إلى المجلد المحلي `.agents/`.
  3. يقوم بمسح وحذف أي مهارات غير مستخدمة (Pruning) لترشيد نافذة السياق 100%.
  4. يولد ملف المانيفست `agent_manifest.json` وسكربت المزامنة المحلي `sync_local_agents.py`.

---

## 3. استيراد الموارد البرمجية عالمياً (الخطاف 20)
...
## 4. بوابات الحراسة الثلاث (The Three Guard Gates)
...
## 5. بروتوكول "الخطوة صفر" وإدارة الذاكرة
...
## 6. المسار المتقدم لإنشاء المهارات والوكلاء الجدد
...
### الخطوة 6: المزامنة السحابية الشاملة (الخطاف 21)
...
## 7. المزامنة الآلية والتحديثات السحابية (الخطاف 21)
