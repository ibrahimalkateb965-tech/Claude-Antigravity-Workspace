...
| **OpenCode CLI** | `qwen2.5-coder:7b` (Local Ollama) / Ox Alpha Unlimited | **Terminal Executor & DB/Remote Layer (No Testing)** | High-speed terminal commands, ORM generation, builds, and remote clients. Zero token cost on local Ollama. |
| **Antigravity CLI** | `hermes3:8b` (Local Ollama) / Gemini 3.7 Flash | **CLI Automation & Code Generator** | Agentic workflows, fast batch generation, and CLI automated tasks. |
| **Antigravity IDE** | Gemini 3.7 Flash (High Reasoning) | **Workspace Intelligence & App Shell** | Navigation, UI Design Systems, Domain Logic, Persistent Memory, and Guidelines. |
