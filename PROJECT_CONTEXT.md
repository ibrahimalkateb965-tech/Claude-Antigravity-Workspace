...
| **نظام الخطافات التلقائي (Auto-Hooks)** | 22 خطافاً تلقائياً موجهاً بالمحفزات | استجابة فورية وتفعيل تلقائي للوكلاء بناءً على الكلمات المفتاحية دون الحاجة لإعادة توجيه يدوي. |
| **خطاف التخصيص والتصدير الذكي (Hook 3)** | `[code-architect]` + `project_agent_tailor.py` | خطاف مخصص مستقل (الخطاف رقم 3) لتحليل سياق المشروع المعتمد وتصدير حزم الوكلاء محلياً مع التنظيف التلقائي (Pruning). |
| **قيادة وإدارة الأسطول التشاركي (Hook 22)** | `[fleet-orchestrator]` + `fleet_config.json` | حوكمة أسطول الـ CLIs (Claude Opus Max للمراجعات الحصرية والاختبارات، OpenCode للمهام الطرفية، Antigravity للواجهات والذاكرة). |
...
| **محرك المزامنة السحابية والمحلية (Ecosystem & Local Sync)** | `sync_global_ecosystem.py` + `sync_local_agents.py` | مطابقة وتحديث كافة الإضافات والمهارات وتوفير مزامنة محلية سريعة للمشاريع الفرعية. |
...
│   ├── HOOKS_GUIDE.md           # الدليل الفني للخطافات التلقائية الـ 22
...
├── fleet_templates/                 # قوالب بروتوكولات الأسطول التشاركي الـ 5
├── fleet_config.json                # مصفوفة توزيع المهام والمسؤوليات للأسطول
...
├── project_agent_tailor.py          # محرك التخصيص والتصدير الذكي والتنظيف للبيئات المحلية
...
5. **جودة الاختبارات واحتكارها لـ Claude Code:** رفض الاختبارات التكرارية التافهة وتفويض كافة الاختبارات التلقائية حصرياً لـ Claude Code CLI بنموذج Opus Max.
...
* **المكونات النشطة والمكتملة:**
  - [x] منظومة الخطافات التلقائية الـ 22 الموثقة بالكامل في `HOOKS_GUIDE.md` و `HOOKS_GUIDE.xlsx`.
  - [x] تخصيص الخطاف رقم 3 كخطاف مستقل للتخصيص والتصدير الذكي لطاقم وموارد المشاريع.
  - [x] تفعيل الخطاف رقم 22 لإدارة وحوكمة أسطول الـ CLIs التشاركي.
  - [x] طاقم الوكلاء المتخصصين الـ 25+ في مجلد `.agents/Sub_Agent/`.
  - [x] مكتبة المهارات البرمجية العالمية (80+ مهارة) في `.agents/skills/`.
  - [x] تطبيق مكتبة الأوامر التفاعلية الديناميكية `03_Dynamic_Prompt_Library/index.html` محدث بـ 22 خطافاً.
  - [x] محرك المزامنة التلقائية السحابية `sync_global_ecosystem.py` ومحرك التخصيص والتنظيف المحلي `project_agent_tailor.py`.
...
```mermaid
flowchart LR
    A["المستخدم / المطور"] -->|محفز الخطاف| B["نظام الخطافات Auto-Hooks (22 خطافاً)"]
